...
void MemoryTest(void)
{
    ...
    unsigned int Start, End, progress, i, num_iter, test_size_b=4;

    unsigned int progress_leds;
    ...
    for(i = 0; i < num_iter; i++)
    {
        ...
        progress = (i + 1) * 100 / num_iter;
        progress_leds = ((1 << (progress * 18 / 100)) - 1);
        PortA = (progress_leds & (0xFF));
        PortB = (progress_leds & (0xFF << 8)) >> 8;
        PortC = (progress_leds & (0x3 << 16)) >> 16;
        ...
        *RamPtr = testValue | (unsigned int)((PortB << 8) | (PortA));
        ...
        {
            printf("\r\nFailed at %08X.  Read: %08x, Expected: %08x", RamPtr, *RamPtr, testValue);
            main();
        }
    }
...
// clear memory in static ram at address 00010000 for 16k
    ...
    // otherwis start the debug monitor

    //Oline0(BugMessage) ;
    //Oline1("By: PAAAAUUUL D") ;
    Oline0("Memory Test") ;
    Oline1("") ;
